...
public class Bulldozer extends Enemy{
	...
	public void contactSprite(Sprite other, int delta) {
		if (other instanceof Player) {
			...
			if (this.isGoRight()) {
				if (other.getBoundBox().getLeft() + World.TILESIZE >= 
						this.getBoundBox().getRight()) {
					other.setX(this.getBoundBox().getRight() 
							+ World.TILESIZE / 2);
				}
				if (other.getBoundBox().getLeft() + World.TILESIZE >= 
						App.SCREEN_WIDTH) {
				...
				if (other.getBoundBox().getRight() - World.TILESIZE < 
						this.getBoundBox().getLeft()) {
					other.setX(this.getBoundBox().getLeft() 
							- World.TILESIZE / 2);
				}
